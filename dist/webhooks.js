...
class Webhooks {
    constructor(token, options, updateHandler) {
        ...
        if (!this.options.serverHostname)
            throw new Error('Webhook server hostname is required');
        if (typeof this.options.serverHostname !== 'string')
            throw new Error('Webhook server hostname must be a string');
        if (typeof this.options.serverPort !== 'number')
            throw new Error('Webhook server port must be a number');
        if (!this.options.path)
            throw new Error('Webhook path is required');
        if (typeof this.options.path !== 'string')
            throw new Error('Webhook path must be a string');
    ...
    handleRequest(update, headers) {
        if (this.options.updateVerification) {
            ...
            if (!signature || !this.verifyUpdate(update, signature))
                throw new WebhookError(400, 'Wrong signature');
        }
        this.updateHandler(update);
    }
    verifyUpdate(update, signature) {
        const secret = (0, crypto_1.createHash)('sha256').update(this.token).digest();
        const serializedData = JSON.stringify(update);
        const hmac = (0, crypto_1.createHmac)('sha256', secret).update(serializedData).digest('hex');
